      ...
      {dashboard.criticalAlerts.length > 0 && (
        <Stack spacing={1} sx={{ mb: 3 }}>
          {dashboard.criticalAlerts.map((alert, idx) => {
            const isError = alert.toLowerCase().includes('critical') || alert.toLowerCase().includes('error');
            return (
              <Alert key={idx} severity={isError ? 'error' : 'warning'}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  {isError ? <ErrorIcon /> : <WarningIcon />}
                  <Typography>{alert.replace(/🚨/g, '')}</Typography>
                </Stack>
              </Alert>
            );
          })}
