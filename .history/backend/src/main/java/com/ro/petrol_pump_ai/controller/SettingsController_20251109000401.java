...
public class SettingsController {
    ...
    /**
     * Send Revenue Alert Email Immediately
     * POST http://localhost:8080/api/settings/send-revenue-alert-now
     */
    @PostMapping("/send-revenue-alert-now")
    public ResponseEntity<String> sendRevenueAlertNow() {
        try {
            BusinessSettings settings = businessSettingsService.getSettings();
            if (settings == null) {
                return ResponseEntity.status(400).body("Settings not configured yet");
            ...
            double todayRevenue = (double) dashboard.getOrDefault("todayRevenue", 0.0);
            double avgDaily7Days = (double) dashboard.getOrDefault("avgDaily7Days", 0.0);
            
            // Use average of last 7 days as expected revenue, or 0 if no data
            double expectedRevenue = avgDaily7Days > 0 ? avgDaily7Days : todayRevenue;

            notificationService.sendRevenueAlert(
                settings.getEmailAddress(),
                todayRevenue,
                expectedRevenue
            );
            ...
            return ResponseEntity.ok(
                "✓ Revenue alert sent to: " + settings.getEmailAddress() + "\n" +
                "Today's Revenue: ₹" + String.format("%.2f", todayRevenue) + "\n" +
                "Expected Revenue: ₹" + String.format("%.2f", expectedRevenue)
            );
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error: " + e.getMessage());
        }
