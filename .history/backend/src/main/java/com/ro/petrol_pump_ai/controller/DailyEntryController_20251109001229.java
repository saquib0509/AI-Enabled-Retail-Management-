...
public class DailyEntryController {
    ...
    @GetMapping
public ResponseEntity<List<DailyEntry>> getAllEntries(
    @RequestParam(defaultValue = "10") int limit) {
    try {
        List<DailyEntry> entries = dailyEntryService.getLastNEntries(limit);
...
@PutMapping("/{id}")
public ResponseEntity<DailyEntry> updateEntry(
    @PathVariable Long id,
    @RequestBody DailyEntry entry) {
    try {
        DailyEntry updated = dailyEntryService.updateEntry(id, entry);
...
public ResponseEntity<String> deleteEntry(@PathVariable Long id) {
    try {
        ...
        return ResponseEntity.status(500).body("Failed to delete entry");
    }
}
