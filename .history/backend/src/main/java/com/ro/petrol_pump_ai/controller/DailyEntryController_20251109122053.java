...
import com.ro.petrol_pump_ai.service.PdfExportService;
...
public class DailyEntryController {
    ...
    @GetMapping
    public ResponseEntity<?> getLastEntries(
        @RequestParam(defaultValue = "10") int limit) {
        try {
            List<DailyEntryResponse> responses = dailyEntryService.getLastNEntries(limit);
    ...
    public ResponseEntity<?> getEntriesByDateRange(
        ...
        try {
            List<DailyEntryResponse> responses = dailyEntryService.getEntriesByDateRange(fromDate, toDate);
            return ResponseEntity.ok(responses);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("{\"error\": \"" + e.getMessage() + "\"}");
    ...
    public ResponseEntity<?> exportToPdf(
        ...
        @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate) {
        try {
            byte[] pdfBytes = pdfExportService.generateEntriesPdf(fromDate, toDate);
            return ResponseEntity.ok()
                .header("Content-Disposition", "attachment; filename=daily-entries.pdf")
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateDailyEntry(
